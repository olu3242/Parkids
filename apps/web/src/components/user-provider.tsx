...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshUser = async () => {
    ...
    if (nextSession?.user) {
      ...
      const validHouseholds = new Set(nextMemberships.map((m) => m.household_id));
      const storedHouseholdId = typeof window !== 'undefined'
        ? window.localStorage.getItem(ACTIVE_HOUSEHOLD_STORAGE_KEY)
        : null;
      const fallbackHouseholdId = nextProfile?.household_id ?? nextMemberships[0]?.household_id ?? null;
      const nextActiveHouseholdId = storedHouseholdId && validHouseholds.has(storedHouseholdId)
        ? storedHouseholdId
        : fallbackHouseholdId;
  ...
  const value = useMemo<UserContextValue>(() => ({
    ...
    memberships,
    activeHouseholdId,
    role: memberships.find((m) => m.household_id === activeHouseholdId)?.role ?? profile?.role ?? null,
